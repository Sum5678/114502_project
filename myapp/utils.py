...
from myapp.models import PemapAll, StoreAll, AbuseReport, StoreAdHistory

def get_unreviewed_counts():
    """回傳事件、商家、交流區、廣告的未審核數量"""
    ...
    # ⚡ 新增：廣告待審核數量
    unreviewed_ads_count = StoreAdHistory.objects.filter(status="pending").count()
    ...
    return {
        ...
        'unreviewed_ads_count': unreviewed_ads_count,  # ⚡ 回傳給前端
    }
